...
if (targetFiles.length !== expectedTargetCount) {
  failures.push(
    `target corpus count drift: expected ${expectedTargetCount}, found ${targetFiles.length}`,
  );
}
...
for (const file of targetFiles) {
  ...
  if (!draftMatch && !canonicalMatch) {
    failures.push(
      `${repoPath(file)}: target document does not have DRAFT-CANONICAL or CANONICAL status`,
    );
    continue;
  }
...
if (existsSync(manifestPath)) {
  ...
  if (!original.includes('> Status: `CANONICAL / R11`'))
    failures.push('existing canonical manifest has unexpected status');
} else {
...
const packagePath = join(repoRoot, 'package.json');
{
  ...
  const desired =
    'node scripts/check-architecture-references.mjs && node scripts/check-architecture-r10.mjs && node scripts/check-architecture-r11.mjs';
  if (
