...
function GlobalStats({
  ...
  const topScorers = useMemo(() => {
    ...
    return Object.entries(map)
      ...
      .map(([id, pts]) => ({
        name:
          players.find((p) => p.id === id)?.apelido ||
          players.find((p) => p.id === id)?.nome?.split(' ')[0] ||
          '—',
        pts,
      }));
  ...
  const sessionsData = useMemo(() => {
    return sessions
      ...
      .map((s) => {
        ...
        const label = new Date(s.date + 'T12:00:00').toLocaleDateString('pt-BR', {
          day: '2-digit',
          month: '2-digit',
        });
  ...
  const playerWinRates = useMemo(() => {
    return players
      .map((player) => {
        ...
        const pg = registeredGames.filter(
        ...
        <div className="card card-border bg-base-200 p-6 rounded-xl min-w-0 overflow-hidden">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={topScorers} margin={{ top: 0, right: 0, bottom: 0, left: -20 }}>
              <XAxis
                dataKey="name"
                tick={{ fill: '#9ca3af', fontSize: 9, fontWeight: 700, textTransform: 'uppercase' }}
                axisLine={false}
                tickLine={false}
              />
        ...
        <div className="card card-border bg-base-200 p-6 rounded-xl min-w-0 overflow-hidden">
          ...
          <ResponsiveContainer width="100%" height={180}>
            <LineChart data={sessionsData} margin={{ top: 0, right: 10, bottom: 0, left: -20 }}>
              ...
              <Legend
                wrapperStyle={{ fontSize: 9, textTransform: 'uppercase', color: '#9ca3af' }}
              />
              <Line
                type="monotone"
                dataKey="jogos"
            ...
            >
              ...
              <Bar dataKey="winRate" name="Win Rate %" radius={[0, 4, 4, 0]}>
                {playerWinRates.map((p, i) => (
                  <Cell
                    key={i}
                    fill={p.winRate >= 60 ? '#10b981' : p.winRate >= 40 ? '#f59e0b' : '#ef4444'}
                  />
                ))}
              </Bar>
            </BarChart>
...
function SessionDetailView({
  ...
  const format =
    session.config?.type === 'tournament' ? (session.config as any).format : 'round_robin';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <h2 className="text-xl font-black uppercase tracking-tight line-clamp-1 px-4">
          {session.name}
        </h2>
        <div className="flex gap-2">
          ...
          <button
            onClick={async () => {
              const ok = await copyToClipboard(getReportText());
      ...
      <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="card card-border bg-base-200 p-6 rounded-xl">
          <h3 className="text-xs font-black uppercase text-accent mb-4 border-b border-accent/10 pb-2">
            Dados e regras
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-[10px] uppercase">
            ...
            <div>
              ...
              <p className="text-white font-black">
                ate {session.config?.maxPoints || 15} pontos -{' '}
        ...
        {session.type === 'tournament' &&
          (format === 'knockout' || format === 'groups_knockout') && (
            <TournamentBracket
              games={games.filter((g) => g.sessionId === session.id)}
              teams={sessionTeams}
            />
          )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  >
                    ...
                    <div className="flex gap-4 items-center">
                      <div className="flex gap-1">
                        ...
                        {blocks > 0 && (
                          <span className="text-[8px] font-black bg-blue-500/20 text-blue-400 px-1 rounded">
                            BLK ×{blocks}
                          </span>
                        )}
                      </div>
          ...
          {session.type === 'tournament' && standings ? (
            <div className="card card-border bg-base-200 p-6 rounded-xl">
              <h3 className="text-xs font-black uppercase text-primary mb-4 border-b border-primary/10 pb-2">
                Classificação Final
              </h3>
              <div className="space-y-3">
                {standings.map((t, i) => {
                  ...
                  return (
                    <div
                      key={t.teamId}
                      className="flex justify-between items-center p-2 bg-white/5 rounded text-[10px] font-mono"
                    >
                      <div className="flex items-center gap-2">
                        <span
                          className={`font-black ${i === 0 ? 'text-accent' : 'text-text-muted'}`}
                        >
                          {i + 1}º
                        </span>
                        <span className="text-xs font-bold text-white uppercase">
                        ...
                        {t.tieBreakerReason && (
                          <span className="text-[8px] text-text-muted/60 uppercase">
                            ({t.tieBreakerReason})
                          </span>
                        )}
                      </div>
                      <div className="flex gap-3 items-center">
                        ...
                        <span className="text-white font-bold">
                          SD {t.pointDifference > 0 ? '+' : ''}
                          {t.pointDifference}
                        </span>
                        <span className="text-accent font-black text-xs">
                          {t.classificationPoints} pts
                        </span>
                      </div>
                    </div>
                  );
                ...
                {teamStats.map((t, i) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span className="font-mono text-text-muted text-[10px] w-4">{i + 1}º</span>
                      <span className="text-xs font-bold text-white uppercase">
                        {teams.find((st) => st.id === t.teamId)?.name || 'Time'}
                      </span>
                    </div>
            ...
            <div className="card card-border bg-base-200 p-6 rounded-xl md:col-span-2">
              ...
              <div className="flex items-center gap-4 p-4 bg-accent/5 rounded-xl border border-accent/20">
                ...
                <div>
                  ...
                  <p className="text-xs font-black text-accent mt-2">
                      ...
                      <thead>
                        <tr>
                          {['#', 'Time', 'J', 'V', 'D', 'PF', 'PC', 'SD', '%', 'Pts'].map((h) => (
                            <th
                              key={h}
                              className="text-left text-[8px] uppercase text-base-content/60 font-black"
                            >
                              {h}
                            </th>
                          ))}
                        ...
                        {standingsA.map((s, i) => {
                          ...
                          return (
                            <tr key={s.teamId}>
                              <td>
                                <span
                                  className={`font-black ${i === 0 ? 'text-accent' : 'text-base-content/60'}`}
                                >
                                  {i + 1}
                                </span>
                              </td>
                              <td>
                                <div className="flex items-center gap-2">
                                  ...
                                  />
                                  <span className="font-black text-white uppercase truncate max-w-[100px] block">
                                    {team?.name}
                                  </span>
                                </div>
                              </td>
                              ...
                              <td
                                className={`font-black ${s.pointDifference > 0 ? 'text-green-400' : s.pointDifference < 0 ? 'text-red-400' : 'text-base-content/60'}`}
                              >
                                {s.pointDifference > 0 ? '+' : ''}
                                {s.pointDifference}
                              </td>
                              <td className="text-base-content/60">{s.winRate}%</td>
                              <td className="text-accent font-black text-sm">
            ...
            {sessionGames
              ...
              .map((game) => {
                ...
                return (
                  <div
                    key={game.id}
                    className="flex items-center justify-between p-3 bg-black/20 rounded border border-white/5 gap-3"
                  >
                    ...
                    <div className="text-[8px] font-mono text-text-muted uppercase">
                      {game.finishedAt
                        ? new Date(game.finishedAt).toLocaleTimeString('pt-BR', {
                            hour: '2-digit',
                            minute: '2-digit',
                          })
                        : '---'}
            ...
            {sessPoints
              ...
              .map((point) => {
                ...
                return (
                  <div
                    key={point.id}
                    className="flex items-center justify-between p-2 bg-black/20 rounded border border-white/5 text-[10px]"
                  >
                    <div>
