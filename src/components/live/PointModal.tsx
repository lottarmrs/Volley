    ...
    <dialog className="modal modal-open">
      <div className="modal-box border border-base-300 p-0 overflow-hidden bg-base-200 w-[calc(100%-2rem)] max-w-md mx-4 sm:mx-auto">
        <div className="p-6 border-b border-base-300 flex justify-between items-center">
          <h3 className="font-bold uppercase tracking-tight text-accent text-base">Registrar Detalhes do Ponto</h3>
        ...
        <div className="p-6 space-y-6">
          <div>
            <label className="text-[10px] font-bold uppercase text-base-content/60 mb-3 block tracking-widest">Responsável pelo Ponto</label>
            <div className="grid grid-cols-2 gap-2">
              {team.playerIds.map(pid => players.find(p => p.id === pid)).filter((p): p is Player => !!p).map(p => (
                <button 
                  key={p.id}
                ...
                >
                  <p className={`text-xs font-bold ${selectedPlayerId === p.id ? 'text-accent' : 'group-hover:text-accent'}`}>{p.nome}</p>
                  <p className="text-[9px] uppercase text-base-content/60">{positionLabels[p.posicaoPrincipal] || 'Jogador'}</p>
                </button>
              ))}
              <button 
          ...
          <div>
            <label className="text-[10px] font-bold uppercase text-base-content/60 mb-3 block tracking-widest">Fundamento Decisivo</label>
            <div className="grid grid-cols-3 gap-2">
              {(Object.entries(POINT_REASON_LABELS) as [PointReason, string][]).map(([key, label]) => (
                 <button 
                   key={key}
