...
export const TournamentActiveView = ({
  ...
  const standingsA = React.useMemo(
    () =>
      groupA
        ? calculateTournamentStandings(
            sessionGames.filter((g) => g.groupId === 'A'),
            groupA.teamIds,
            classPoints,
          )
        : [],
  ...
  const standingsB = React.useMemo(
    () =>
      groupB
        ? calculateTournamentStandings(
            sessionGames.filter((g) => g.groupId === 'B'),
            groupB.teamIds,
            classPoints,
          )
        : [],
  ...
  const standingsShareText = () =>
    formatTournamentStandingsForWhatsApp({
      sessionName: activeSession.name,
      standings,
      teams: sessionTeams,
    });
  const scorersShareText = () =>
    formatTournamentScorersForWhatsApp({
      sessionName: activeSession.name,
      players,
      scoringRanking,
  ...
  return (
    <div className="space-y-6 pb-24">
      ...
      {isPaused && (
        <div
          role="alert"
          className="alert alert-warning alert-soft text-center justify-center font-bold uppercase tracking-[0.1em] text-xs"
        >
          Torneio pausado
        </div>
      )}
      ...
      <div className="stats stats-vertical sm:stats-horizontal shadow bg-base-200 border border-base-300 w-full">
        <div className="stat">
          <div className="stat-title text-[10px] font-bold uppercase text-base-content/60">
            Status
          </div>
        ...
        <div className="stat">
          ...
          <div className="stat-value text-base font-bold uppercase text-base-content">
            Até {activeSession.config?.maxPoints} -{' '}
      ...
      {progress.isComplete ? (
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            ...
            <button
              onClick={async () => {
                const ok = await copyToClipboard(finalShareText());
                if (ok) alert('Resumo copiado!');
              }}
          ...
          teamB ? (
            ...
            >
              ...
              <p className="text-2xl font-bold font-mono text-base-content">
                {currentGame.scoreA} <span className="text-base-content/55">×</span>{' '}
                {currentGame.scoreB}
              </p>
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={async () => {
                    const ok = await copyGameToClipboard(currentGame.id);
                    if (ok) alert('Copiado!');
                  }}
            ...
            <div className="flex flex-wrap justify-center gap-3">
              ...
              <button
                onClick={() => {
                  ...
                  const winner = sessionTeams.find(
                    (t) =>
                      t.id ===
        ...
        <div className="card card-border border-dashed bg-base-200 p-8 rounded-2xl text-center space-y-4">
          ...
          <div>
            ...
            <p className="text-[10px] text-base-content/60 uppercase mt-1">
              Inicie a primeira partida do torneio.
            </p>
          </div>
          <button
            onClick={() => startNextGame(setActiveSession)}
      ...
      <div className="card card-border bg-base-200 rounded-2xl overflow-hidden">
        <div className="p-4 bg-base-300/50 border-b border-base-300 flex items-center justify-between">
          <h4 className="text-[10px] font-bold uppercase tracking-[0.2em] text-base-content/60">
            Sessões do Confronto
          </h4>
          <span className="badge badge-accent badge-soft font-bold uppercase text-[9px]">
            Tempo Real
          </span>
        </div>
        <div className="max-h-48 overflow-y-auto p-2 space-y-1">
          ...
          {sessionPoints.length === 0 && (
            <div className="text-center py-10 text-[9px] text-base-content/60 italic uppercase opacity-30">
...
function GameActions({
  ...
  const editScore = () => {
    const raw = prompt(
      `Corrigir placar final\n\n${teamAName} x ${teamBName}`,
  ...
  return (
    <div className="flex items-center gap-1 flex-wrap justify-end">
      ...
      {game.status === 'scheduled' && (
        <>
          {!isBlocked ? (
            <button onClick={onStart} className="btn btn-xs btn-primary font-bold uppercase">
              Iniciar
            </button>
          ) : (
            <span
              className="btn btn-xs btn-disabled font-bold uppercase cursor-not-allowed"
              title="Aguardando confrontos anteriores"
            >
              Bloqueado
            </span>
          )}
          <button onClick={onMoveUp} className="btn btn-xs btn-outline font-bold">
            ↑
          </button>
          <button onClick={onMoveDown} className="btn btn-xs btn-outline font-bold">
            ↓
          </button>
          <button
            onClick={onShare}
      ...
      {canEdit && (
        <>
          <button onClick={editScore} className="btn btn-xs btn-outline font-bold uppercase">
            Editar
          </button>
          <button
            onClick={() => {
      ...
      {game.status !== 'cancelled' && (
        <button
          onClick={() => {
            if (confirm('Cancelar este jogo?')) onCancel();
          }}
          className="btn btn-xs btn-error btn-soft font-bold uppercase"
        >
          Cancelar
        </button>
      )}
  ...
  return (
    ...
    >
      <div className="card card-border bg-base-200 border-accent/30 p-8 rounded-2xl text-center">
        ...
        <h3 className="text-2xl font-bold uppercase tracking-[0.3em] text-accent">
          Torneio Concluído!
        </h3>
        <p className="text-[10px] text-base-content/60 uppercase font-bold mt-2">
          Classificação final
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {standings.slice(0, 3).map((s, i) => {
          ...
          return (
            <div
              key={s.teamId}
              className={`card card-border bg-base-200 p-6 rounded-xl text-center flex flex-col justify-between ${i === 0 ? 'border-accent bg-accent/5' : ''}`}
            >
              <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="flex items-center gap-4 p-4 bg-accent/10 rounded-xl border border-accent/20 flex-1">
              ...
              <div>
                ...
                <p className="text-xs font-bold text-accent mt-2">
                  {mvp.totalPoints} Pontos Marcados
                </p>
              </div>
            </div>
          </div>
        ...
        {scoringRanking.length > 0 &&
          (() => {
            ...
            const topScorerTeam = sessionTeams.find((t) =>
              t.playerIds.includes(scoringRanking[0].playerId),
            );
            return topScorer ? (
              <div className="card card-border bg-base-200 p-6 rounded-xl flex flex-col justify-between">
                ...
                <div className="flex items-center gap-4 p-4 bg-primary/10 rounded-xl border border-primary/20 flex-1">
                  ...
                  <div>
                    ...
                    <p className="text-[9px] text-base-content/60 uppercase">
                      {topScorerTeam?.name || 'Sem Time'}
                    </p>
                    <p className="text-xs font-bold text-primary mt-2">
                      {scoringRanking[0].points} Pontos Totais
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 bg-base-300/50 rounded-xl border border-base-300">
            <div>
              ...
              <p className="text-sm font-bold text-base-content font-mono">
                {
                  sessionGames.filter((g) => g.status === 'finished' || g.status === 'walkover')
                    .length
                }
              </p>
            </div>
            ...
            <div className="sm:col-span-2 border-t border-base-300 pt-2">
              ...
              <p className="text-xs font-bold text-base-content uppercase font-mono">
                {getHighestScoreMatch(sessionGames, sessionTeams)}
              </p>
            </div>
            <div className="sm:col-span-2 border-t border-base-300 pt-2">
              <p className="text-[8px] font-bold uppercase text-base-content/60">
                Jogo Mais Equilibrado
              </p>
              <p className="text-xs font-bold text-base-content uppercase font-mono">
                {getMostBalancedMatch(sessionGames, sessionTeams)}
              </p>
            </div>
            <div className="sm:col-span-2 border-t border-base-300 pt-2">
              <p className="text-[8px] font-bold uppercase text-base-content/60">
              ...
              <p className="text-xs font-bold text-base-content uppercase">
                {getLongestWinStreak(sessionGames, sessionTeams)}
              </p>
            </div>
          </div>
        </div>
