...
export function GuestPlayerModal({
  ...
  const filteredTemplatePlayers = useMemo(() => {
    ...
    return players
      ...
      .filter(
        (p) =>
          p.nome.toLowerCase().includes(templateSearch.toLowerCase()) ||
  ...
  const handleSave = (editDetails: boolean) => {
    ...
    const newGuest: Player = {
      ...
      ativo: true,
      posicaoPrincipal,
      posicoesSecundarias: selectedTemplatePlayer
        ? [...selectedTemplatePlayer.posicoesSecundarias]
        : [],
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-base-300/40 p-5 rounded-xl border border-base-300">
            ...
            <div className="fieldset md:col-span-1">
              <label className="fieldset-legend text-[10px] font-bold uppercase text-text-muted">
                Nome do Atleta
              </label>
              <input
                type="text"
          ...
          <div className="bg-base-300/40 p-5 rounded-xl border border-base-300 space-y-4">
            <div>
              ...
              <p className="text-[9px] text-text-muted uppercase font-bold mt-1">
                Selecione um atleta cadastrado para servir de base. Você pode ajustar os atributos
                depois.
              </p>
            </div>
                  ...
                  {filteredTemplatePlayers.map((p) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="badge badge-accent font-mono font-black text-xs">
                          {pOverall}
                        </span>
                      </button>
                    );
              ...
              <div className="flex items-center gap-3 bg-accent/10 border border-accent/20 rounded-xl p-3.5 text-xs text-accent">
                ...
                <div>
                  ...
                  <span className="font-mono uppercase font-black">
                    {selectedTemplatePlayer.nome}
                  </span>{' '}
                  <span className="badge badge-accent badge-sm font-mono font-black">
                    {calculateGeneralOverall(selectedTemplatePlayer)} OVERALL
                  </span>
                </div>
              </div>
            )}
                ...
                <div key={attr.key} className="bg-base-300 p-3 rounded-xl border border-base-300">
                  <div className="flex justify-between items-center mb-1">
                    <div className="flex items-center gap-1 shrink-0">
                      <span className="text-[9px] font-bold uppercase text-base-content/80 whitespace-nowrap">
                        {attr.label}
                      </span>
                      <div
                    ...
                    <span className="text-xs font-mono font-bold text-accent whitespace-nowrap">
                      ...
                      <span className="text-[10px] text-base-content/50 uppercase font-sans font-medium">
                        {getAttributeLabel(atributos[attr.key])}
                      </span>
                    </span>
                  </div>
                  <input
