...
export const PlayerEditView = ({
  ...
  const historyData = useMemo(() => {
    if (playerFinishedSessions.length > 0) {
      return playerFinishedSessions.map(s => {
        ...
        const playerTeamsInSession = teams.filter(t => t.sessionId === s.id && t.playerIds.includes(editingPlayer.id));
        const playerTeamIds = playerTeamsInSession.map(t => t.id);
        const playerSessionGames = sessionGames.filter(g => playerTeamIds.includes(g.teamAId) || playerTeamIds.includes(g.teamBId));
        ...
        const winRate = playerSessionGames.length > 0 ? Math.round((wins / playerSessionGames.length) * 100) : 0;
        return {
  ...
  const playerTeamIds = teams.filter(t => t.playerIds.includes(editingPlayer.id)).map(t => t.id);
  const hasHistory = games.some(g => playerTeamIds.includes(g.teamAId) || playerTeamIds.includes(g.teamBId)) ||
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-1 card bg-base-200 border border-base-300 p-4 h-[400px] lg:h-[780px] flex flex-col shadow-xl">
          <h3 className="text-xs font-bold uppercase text-base-content tracking-wider mb-3">Lista de Atletas</h3>
            ...
            {filteredPlayers.map(p => {
              ...
              return (
                <div
                  ...
                  className={`p-3 rounded-xl cursor-pointer transition-all border flex items-center justify-between ${isEditing
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className={`w-8 h-8 rounded-full border flex items-center justify-center font-bold text-xs ${isEditing ? 'border-primary bg-primary/20 text-primary' : 'border-base-300 bg-base-100 text-base-content/50'
                      }`}>
                      {p.nome.substring(0, 2).toUpperCase()}
                    </div>
                    <div>
                      <p className="text-xs font-bold uppercase tracking-tight">{p.apelido || p.nome}</p>
                      <span className="text-[9px] text-base-content/50 font-mono uppercase">
                        {positionAbbreviations[p.posicaoPrincipal]} • {p.genero}
                      </span>
                    </div>
                  </div>
                  <span className={`font-mono font-black text-xs ${isEditing ? 'text-accent' : 'text-base-content/40'}`}>{pOverall}</span>
                </div>
              );
            })}
          </div>
        ...
        <div className="lg:col-span-2 card bg-base-200 border border-base-300 p-6 h-auto lg:h-[780px] flex flex-col justify-between shadow-xl">
          <div className="space-y-5 flex-1 overflow-y-auto pr-2 custom-scrollbar">
            ...
            <div className="flex items-start gap-4 border-b border-base-300 pb-4">
              <div className="avatar avatar-placeholder shrink-0">
                <div className="w-16 rounded-full bg-base-300 text-accent border-2 border-accent font-black text-xl shadow-lg shadow-accent/15">
                  <span>{editingPlayer.nome ? editingPlayer.nome.substring(0, 2).toUpperCase() : 'AT'}</span>
                </div>
              </div>
              <div className="flex-1 min-w-0 space-y-2">
                <div className="flex flex-col sm:flex-row gap-2 w-full">
                  <div className="flex-1">
                    ...
                    />
                    {validationErrors.nome && <p className="text-[9px] text-error font-bold uppercase mt-1">{validationErrors.nome}</p>}
                  </div>
                  <div className="w-full sm:w-1/3">
                    <input
                      type="text"
                      value={editingPlayer.apelido || ''}
                      onChange={(e) => setEditingPlayer({ ...editingPlayer, apelido: e.target.value })}
                      placeholder="Apelido"
                      className="input input-bordered input-sm w-full font-bold uppercase text-base-content"
                    />
                  </div>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-[9px] text-base-content/40 font-mono uppercase w-14 shrink-0">PRINCIPAL</span>
                  {(Object.keys(positionAbbreviations) as Position[]).map(pos => {
                    const isPrimary = pos === editingPlayer.posicaoPrincipal;
                    return (
                      <label key={pos} className={`px-1.5 py-0.5 rounded border text-[9px] font-bold cursor-pointer uppercase select-none transition-colors ${isPrimary
                        ...
                        <input
                          ...
                          onChange={() => {
                            setEditingPlayer({
                              ...
                              posicoesSecundarias: editingPlayer.posicoesSecundarias.filter(p => p !== pos)
                            });
                          }}
                          className="hidden"
                        />
                  ...
                  <span className={`badge badge-xs font-black uppercase font-mono ml-auto ${overall > 75 ? 'badge-warning' : overall > 60 ? 'badge-primary' : overall > 45 ? 'badge-secondary' : 'badge-ghost'
                    }`}>
                    {overall > 75 ? 'ELITE' : overall > 60 ? 'COMPETITIVO' : overall > 45 ? 'SOCIAL' : 'INICIANTE'}
                  </span>
                </div>

                {/* Secondary positions — checkbox pill toggles */}
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-[9px] text-base-content/40 font-mono uppercase w-14 shrink-0">SECUND.</span>
                  {(Object.keys(positionAbbreviations) as Position[]).map(pos => {
                    if (pos === editingPlayer.posicaoPrincipal) return null;
                    const isSelected = editingPlayer.posicoesSecundarias.includes(pos);
                    return (
                      <label key={pos} className={`px-1.5 py-0.5 rounded border text-[9px] font-bold cursor-pointer uppercase select-none transition-colors ${isSelected ? 'bg-primary/20 border-primary text-primary' : 'bg-base-300 border-base-300 text-base-content/50 hover:border-base-content/20'
                        }`}>
                        <input
                          type="checkbox"
                          checked={isSelected}
            ...
            {editingPlayer.isGuest && (
              <div className="flex items-center justify-between bg-primary/10 border border-primary/20 p-4 rounded-xl shadow-inner mb-4">
                <div className="space-y-0.5">
                  <span className="text-[9px] font-black uppercase text-primary font-mono tracking-wider">Atleta Convidado</span>
                  <p className="text-[10px] text-base-content/70">Este atleta está cadastrado como convidado temporário.</p>
                </div>
                <button
                  type="button"
            ...
            <div className="bg-base-300/40 p-4 rounded-xl border border-base-300 space-y-4">
              <span className="text-[9px] font-bold text-base-content/40 uppercase block mb-1">Informações Básicas e Físicas</span>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {/* Genero */}
                <div className="flex flex-col gap-1">
                  <label className="text-[8px] font-bold text-base-content/60 uppercase">Gênero</label>
                  <select
                    value={editingPlayer.genero}
                    onChange={(e) => setEditingPlayer({ ...editingPlayer, genero: e.target.value as any })}
                ...
                {/* Altura */}
                <div className="flex flex-col gap-1">
                  <label className="text-[8px] font-bold text-base-content/60 uppercase">Altura (CM)</label>
                  <input
                    type="number"
                    value={editingPlayer.alturaCm || ''}
                    onChange={(e) => setEditingPlayer({ ...editingPlayer, alturaCm: e.target.value ? parseInt(e.target.value) : undefined })}
                    className="input input-bordered input-xs w-full uppercase font-mono font-bold"
                    placeholder="Altura"
                  />
                  {validationErrors.alturaCm && <p className="text-[8px] text-error font-bold uppercase">{validationErrors.alturaCm}</p>}
                </div>
                {/* Mao Dominante */}
                <div className="flex flex-col gap-1">
                  <label className="text-[8px] font-bold text-base-content/60 uppercase">Mão Dominante</label>
                  <select
                    value={editingPlayer.maoDominante}
                    onChange={(e) => setEditingPlayer({ ...editingPlayer, maoDominante: e.target.value as any })}
                    className="select select-bordered select-xs w-full uppercase font-bold"
                  >
                ...
                {/* Status Ativo */}
                <div className="flex flex-col gap-1">
                  <label className="text-[8px] font-bold text-base-content/60 uppercase">Status Geral</label>
                  <select
                    value={editingPlayer.ativo ? 'ativo' : 'inativo'}
                    onChange={(e) => setEditingPlayer({ ...editingPlayer, ativo: e.target.value === 'ativo' })}
                    className="select select-bordered select-xs w-full uppercase font-bold"
                  >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 border-t border-base-300 pt-3">
                <div className="flex items-center justify-between bg-base-300 p-2 rounded border border-base-300">
                  <span className="text-[9px] font-bold uppercase text-base-content/70">Atleta Lesionado</span>
                  <input
                    type="checkbox"
                ...
                <div className="flex items-center justify-between bg-base-300 p-2 rounded border border-base-300">
                  <span className="text-[9px] font-bold uppercase text-base-content/70">Presença Frequente</span>
                  <input
                    type="checkbox"
              ...
              {editingPlayer.status.lesionado && (
                <div className="flex flex-col gap-1 border-t border-base-300 pt-3">
                  <label className="text-[8px] font-bold text-error uppercase">Limitação Física / Observação de Lesão</label>
                  <input
                    type="text"
                    value={editingPlayer.status.limitacaoFisica || ''}
                    onChange={(e) => setEditingPlayer({
                      ...editingPlayer,
                      status: { ...editingPlayer.status, limitacaoFisica: e.target.value || null }
                    })}
            ...
            <div className="bg-base-300/40 p-4 rounded-xl border border-base-300 space-y-3">
              <span className="text-[9px] font-bold text-base-content/40 uppercase block">Perfil do Atleta</span>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="flex flex-col gap-1 bg-base-300/50 p-2.5 rounded-lg border border-base-300/50">
                  <span className="text-[8px] font-bold text-success uppercase">★ Especialidade</span>
            ...
            <div className="bg-base-300/40 p-4 rounded-xl border border-base-300 space-y-4">
              <span className="text-[9px] font-bold text-base-content/40 uppercase block mb-1">Forma Física Atual</span>

              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {/* Valor Forma */}
                <div className="flex flex-col gap-1 col-span-1">
                  <label className="text-[8px] font-bold text-base-content/60 uppercase">Rating de Forma (-5 a 5)</label>
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      onChange={(e) => setEditingPlayer({
                        ...editingPlayer,
                        formaAtual: { ...editingPlayer.formaAtual, valor: parseFloat(e.target.value) }
                      })}
                    ...
                    <span className="text-xs font-mono font-bold text-accent min-w-[20px] text-center">{editingPlayer.formaAtual.valor}</span>
                  </div>
                </div>
                {/* Observacao Forma */}
                <div className="flex flex-col gap-1 col-span-2">
                  <label className="text-[8px] font-bold text-base-content/60 uppercase">Observação sobre a Forma</label>
                  <input
                    type="text"
            ...
            <div className="space-y-4">
              <h4 className="text-[10px] font-bold uppercase tracking-widest text-base-content/60">Editor de Atributos Principais</h4>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[
                  ...
                  { label: 'Stamina / Resistência', key: 'resistencia' as keyof Attributes, placement: 'tooltip-left' }
                ].map(attr => (
                  <div key={attr.key} className="bg-base-300 p-3.5 rounded-xl border border-base-300 transition-colors">
                    <div className="flex justify-between items-center mb-1.5">
                      <div className="flex items-center gap-1 shrink-0">
                        <span className="text-[10px] font-bold uppercase text-base-content/70 tracking-wider whitespace-nowrap">{attr.label}</span>
                      ...
                      <span className="text-xs font-mono font-bold text-accent whitespace-nowrap">
                        {editingPlayer.atributos[attr.key]} · <span className="text-[10px] text-base-content/50 uppercase font-sans font-medium">{getAttributeLabel(editingPlayer.atributos[attr.key])}</span>
                      </span>
                    </div>
                    <input
                      type="range"
                      ...
                      onChange={(e) => setEditingPlayer({
                        ...editingPlayer,
                        atributos: { ...editingPlayer.atributos, [attr.key]: parseFloat(e.target.value) }
                      })}
              ...
              <div className="bg-base-300 p-4 rounded-xl border border-base-300 mt-3">
                <span className="text-[9px] font-bold text-base-content/40 uppercase block mb-3">Outros Atributos Técnicos</span>
                    ...
                    <div key={attr.key} className="flex flex-col gap-1">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-[9px] font-mono font-bold text-primary whitespace-nowrap">
                          {editingPlayer.atributos[attr.key]} · <span className="text-[8px] text-base-content/40 uppercase font-sans font-medium">{getAttributeLabel(editingPlayer.atributos[attr.key])}</span>
                        </span>
                      </div>
                      <input
                        type="range"
            ...
            <div className="space-y-3 border-t border-base-300 pt-4">
              <h4 className="text-[10px] font-bold uppercase tracking-widest text-base-content/60">Histórico de Performance</h4>
              <div className="bg-base-300 p-4 rounded-xl border border-base-300 min-w-0 overflow-hidden">
                <ResponsiveContainer width="100%" height={100}>
                  <LineChart data={historyData} margin={{ top: 5, right: 5, bottom: 5, left: -25 }}>
                    ...
                    <Line type="monotone" dataKey="forma" stroke="var(--color-accent)" strokeWidth={2} dot={{ r: 3, fill: 'var(--color-accent)' }} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
        ...
        <div className="lg:col-span-1 card bg-base-200 border border-base-300 p-6 h-auto lg:h-[780px] flex flex-col justify-between shadow-xl min-w-0 overflow-hidden">
          <div className="space-y-4">
            <h3 className="text-xs font-bold uppercase text-base-content tracking-wider">Radar de Habilidades</h3>
            <p className="text-[10px] text-base-content/65 leading-relaxed">Gráfico técnico de equilíbrio do atleta por fundamento.</p>
          </div>
          ...
          <div className="flex-1 flex items-center justify-center">
            <ResponsiveContainer width="100%" height={280}>
              <RadarChart cx="50%" cy="50%" outerRadius="75%" data={radarData}>
                ...
                <PolarRadiusAxis angle={30} domain={[0, 100]} stroke="var(--color-text-subtle)" fontSize={8} />
                <Radar name={editingPlayer.apelido || editingPlayer.nome} dataKey="A" stroke="var(--color-accent)" fill="var(--color-accent)" fillOpacity={0.35} />
              </RadarChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="bg-base-300 p-4 rounded-xl border border-base-300">
              <div className="flex items-end justify-between">
                <div>
                  <span className="text-[10px] font-bold text-base-content/60 uppercase">Overall</span>
                  <p className="text-3xl font-black font-mono text-accent mt-0.5">{overall}</p>
                </div>
                <div className="text-right">
                  <p className="text-[8px] text-base-content/40 font-bold uppercase">Influência da Forma</p>
                  <div className="flex items-center gap-1 justify-end mt-0.5">
                    <span className="text-[10px] font-mono text-base-content/50">base {rawOverall}</span>
                    {overall !== rawOverall && (
                      <span className={`text-[11px] font-black font-mono ${overall > rawOverall ? 'text-success' : 'text-error'}`}>
                        {overall > rawOverall ? '+' : ''}{overall - rawOverall}
                      </span>
                    )}
                  </div>
                  <div className="flex items-center gap-1 mt-0.5">
                    <progress className="progress progress-accent w-16 h-1.5" value={editingPlayer.formaAtual.valor + 5} max={10} />
                    <span className="text-[9px] font-mono font-bold text-accent">{editingPlayer.formaAtual.valor}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-base-300/60 rounded-xl border border-base-300 p-3 space-y-2">
              <p className="text-[8px] font-bold uppercase tracking-widest text-base-content/40">Overall por Posição</p>
              {(['levantador', 'oposto', 'ponteiro', 'central', 'libero', 'all-rounder'] as const).map(pos => {
                ...
                return (
                  <div key={pos} className={`flex items-center justify-between text-[9px] font-mono ${isPrimary ? 'text-accent' : isSecondary ? 'text-primary' : 'text-base-content/30'
                    }`}>
                    <span className="uppercase font-bold">{positionLabels[pos]}</span>
                    <div className="flex items-center gap-2">
                      <progress
                        className={`progress w-16 h-1 ${isPrimary ? 'progress-accent' : isSecondary ? 'progress-primary' : 'progress-neutral'
                          }`}
                        value={posOverall}
                        max={100}
                      />
