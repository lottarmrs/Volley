...
export function useCommunityPresence() {
  ...
  const getPresence = useCallback((communityId: string) => {
    return presenceRecords.find(record => record.communityId === communityId && record.date === today()) || null;
  }, [presenceRecords]);

  const ensurePresence = useCallback((communityId: string) => {
  ...
  const upsertPresence = useCallback((next: CommunityPresence) => {
    setPresenceRecords(prev => {
      const exists = prev.some(record => record.communityId === next.communityId && record.date === next.date);
      return exists
        ? prev.map(record => record.communityId === next.communityId && record.date === next.date ? next : record)
        : [...prev, next];
    });
  }, []);

  const setPresenceStatus = useCallback((communityId: string, playerId: string, status: CommunityPresenceStatus) => {
  ...
  const getPresentPlayers = useCallback((communityId: string, players: Player[]) => {
    const presence = getPresence(communityId);
    return players.filter(player => presence?.items.some(item => item.playerId === player.id && item.status === 'present'));
  }, [getPresence]);
  ...
  return useMemo(() => ({
    ...
    addGuest,
    getPresentPlayers,
  }), [presenceRecords, getPresence, setPresenceStatus, clearPresence, selectFrequentPlayers, useLastPresence, addGuest, getPresentPlayers]);
}
