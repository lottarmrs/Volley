...
import {
  Session, Game, PointEvent, Team, Player,
  FreePlayConfig, PointReason, GameReport
} from '../types';
...
export function useLiveSession(
  ...
  const lastResultGame = useMemo(
    () => [...sessionGames].filter(isResultGame).sort((a, b) => (b.sequenceNumber || 0) - (a.sequenceNumber || 0))[0],
    [sessionGames]
  );
  ...
  const currentGame = activeGame ?? (activeSession?.type === 'tournament' ? lastResultGame : lastGame);
  ...
  }, [sessionGames, activeSession?.type, activeSession?.teamIds, activeSession?.config, sessionTeams]);
  ...
  // ── Register point ────────────────────────────────────────────────────────
  const registerPoint = useCallback((teamId: string, playerId?: string, reason?: PointReason) => {
    if (!currentGame || !activeSession || currentGame.status !== 'active' || isRegisteringPoint.current) return;
    ...
    try {
      ...
      if (winnerSymbol) {
        ...
        updatedGame.winnerTeamId = winnerSymbol === 'A' ? currentGame.teamAId : currentGame.teamBId;
        updatedGame.loserTeamId  = winnerSymbol === 'A' ? currentGame.teamBId : currentGame.teamAId;
        updatedGame.finishedAt   = new Date().toISOString();
        updatedGame.finishReason = 'auto';
        const report = generateGameReport(updatedGame, [...pointEvents, pointEvent], sessionTeams, players);
        setGameReports(prev => [...prev, report]);
      }

      setGames(prev => prev.map(g => g.id === updatedGame.id ? updatedGame : g));
    } finally {
      setTimeout(() => { isRegisteringPoint.current = false; }, 200);
    }
  }, [currentGame, activeSession, sessionTeams, sessionPoints, pointEvents, players, setPointEvents, setGames, setGameReports]);
  ...
  const finishCurrentGameManually = useCallback(() => {
    ...
    const winnerTeamId = currentGame.scoreA > currentGame.scoreB ? currentGame.teamAId : currentGame.teamBId;
    const loserTeamId = currentGame.scoreA > currentGame.scoreB ? currentGame.teamBId : currentGame.teamAId;
    const updatedGame: Game = {
      ...currentGame,
      status: 'finished',
      winnerTeamId,
    ...
    setGameReports(prev => prev.some(r => r.gameId === updatedGame.id) ? prev : [...prev, report]);
    setGames(prev => prev.map(g => g.id === updatedGame.id ? updatedGame : g));
  ...
  const startNextGame = useCallback((updateActiveSession: (s: Session) => void) => {
    ...
    if (activeSession.type === 'free_play' && activeSession.config?.type === 'free_play') {
      ...
      updateActiveSession({
        ...activeSession,
        config: { ...activeSession.config!, initialCourtTeams: result.nextCourtTeams, initialQueue: result.nextQueue },
      });
  ...
  const nextMatchPreview = useMemo(() => {
    ...
    if (activeSession.type !== 'free_play' || activeSession.config?.type !== 'free_play') return null;
  ...
  const registerWalkover = useCallback((gameId: string, winnerTeamId: string) => {
    if (!activeSession?.config) return;
    const pointsPerGame = activeSession.config.maxPoints || 15;
    setGames(prev => prev.map(game => {
      if (game.id !== gameId || !['scheduled', 'active', 'paused'].includes(game.status)) return game;
      return createWalkoverResult(game, winnerTeamId, pointsPerGame);
    }));
  }, [activeSession?.config, setGames]);

  const pauseGame = useCallback((gameId: string, paused: boolean) => {
    setGames(prev => prev.map(game => {
  ...
  const cancelGame = useCallback((gameId: string) => {
    setGames(prev => prev.map(game => game.id === gameId
      ? { ...game, status: 'cancelled' as const, finishedAt: new Date().toISOString(), finishReason: null }
      : game
    ));
  }, [setGames]);

  const updateFinalScore = useCallback((gameId: string, scoreA: number, scoreB: number) => {
  ...
  const reorderScheduledGame = useCallback((gameId: string, direction: 'up' | 'down') => {
    setGames(prev => {
      ...
      if (sessionOrdered[index].status !== 'scheduled' || sessionOrdered[targetIndex].status !== 'scheduled') return prev;
      [sessionOrdered[index], sessionOrdered[targetIndex]] = [sessionOrdered[targetIndex], sessionOrdered[index]];
      return [
        ...otherGames,
  ...
  // ── Share helpers ──────────────────────────────────────────────────────────
  const shareGameToWhatsApp = useCallback((gameId: string) => {
    const game = games.find(g => g.id === gameId);
    if (!game) return;

    if (game.status === 'scheduled') {
      openWhatsAppShare(formatScheduledMatchForWhatsApp({
        game,
        teams: sessionTeams,
    ...
    openWhatsAppShare(formatTournamentGameForWhatsApp({ game, teams: sessionTeams, players, points: pointEvents }));
  }, [gameReports, games, sessionTeams, players, pointEvents, activeSession?.name]);

  const copyGameToClipboard = useCallback(async (gameId: string) => {
    const game = games.find(g => g.id === gameId);
    if (!game) return false;

    if (game.status === 'scheduled') {
      return copyToClipboard(formatScheduledMatchForWhatsApp({
        game,
        teams: sessionTeams,
        sessionName: activeSession?.name || 'Sessão'
      }));
    ...
    return copyToClipboard(formatTournamentGameForWhatsApp({ game, teams: sessionTeams, players, points: pointEvents }));
  }, [gameReports, games, sessionTeams, players, pointEvents, activeSession?.name]);
