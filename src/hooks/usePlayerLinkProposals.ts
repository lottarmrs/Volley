...
export function usePlayerLinkProposals(
  ...
  const handleProposePlayerLink = useCallback(async (playerId: string) => {
    ...
    setLinkProposals(prev => [
      ...prev.filter(p => !(p.playerId === playerId && p.userId === currentUserId && p.status === 'pending')),
      newProposal
    ]);
    ...
    try {
      if (player.cloudId) {
        ...
        setLinkProposals(prev =>
          prev.map(p =>
            p.id === tempProposalId
              ? { ...p, id: cloudProposalId, syncStatus: 'synced', lastSyncedAt: new Date().toISOString() }
              : p
          )
        );
      }
  ...
  const handleReviewPlayerLink = useCallback(async (proposalId: string, action: 'approve' | 'reject') => {
    if (!currentUserId) throw new Error('Usuário não autenticado.');

    const proposal = linkProposals.find(p => p.id === proposalId);
    if (!proposal) throw new Error('Solicitação não encontrada.');

    const player = players.find(p => p.id === proposal.playerId || (p.cloudId && p.cloudId === proposal.playerCloudId));
    ...
    if (action === 'approve') {
      ...
      setLinkProposals(prev =>
        prev.map(p => {
          ...
          if ((p.playerId === player.id || p.userId === proposal.userId) && p.status === 'pending') {
            return {
              ...p,
              status: 'superseded',
              reviewedBy: currentUserId,
    ...
    if (!isTemp) {
      try {
        ...
        console.error(`Erro ao ${action === 'approve' ? 'aprovar' : 'rejeitar'} proposta na nuvem:`, error);
      }
    }
  }, [players, setPlayers, linkProposals, currentUserId]);

  const handleCancelPlayerLink = useCallback(async (proposalId: string) => {
    if (!currentUserId) throw new Error('Usuário não autenticado.');
  ...
  const handleUnlinkPlayer = useCallback(async (playerId: string) => {
    if (!currentUserId) throw new Error('Usuário não autenticado.');
    ...
    setLinkProposals(prev =>
      prev.map(p => {
        if ((p.playerId === playerId || (p.playerCloudId && player.cloudId && p.playerCloudId === player.cloudId)) && p.status === 'pending') {
          return {
            ...p,
            status: 'superseded',
            reviewedBy: currentUserId,
