...
import { Session, Team, Game, PointEvent, GameReport, SessionReport, TournamentConfig } from '../types';
import {
  STORAGE_KEYS,
  loadFromStorage,
  saveToStorage
...
export function useSessions() {
  const [sessions, setSessions]               = useState<Session[]>(() => normalizeSessions(loadFromStorage(STORAGE_KEYS.sessions, [])));
  ...
  const [games, setGames]                     = useState<Game[]>(() => normalizeGames(loadFromStorage(STORAGE_KEYS.games, [])));
  const [pointEvents, setPointEvents]         = useState<PointEvent[]>(() => loadFromStorage(STORAGE_KEYS.points, []));
  const [gameReports, setGameReports]         = useState<GameReport[]>(() => loadFromStorage(STORAGE_KEYS.gameReports, []));
  const [sessionReports, setSessionReports]   = useState<SessionReport[]>(() => loadFromStorage(STORAGE_KEYS.sessionReports, []));
  ...
  useEffect(() => {
    ...
    if (cleanSessionReports.length !== sessionReports.length) setSessionReports(cleanSessionReports);
  ...
  useEffect(() => {
    ...
    const propagated = propagateKnockoutResults(games, activeSession.id, activeSession.config as TournamentConfig);
  ...
  return {
    ...
    teams, setTeams,
    games, setGames,
    pointEvents, setPointEvents,
    gameReports, setGameReports,
    sessionReports, setSessionReports,
  };
}
