...
export function getCommunitySummary(params: {
  ...
  const finishedGames = communityGames.filter(
    (game) => game.status === 'finished' || game.status === 'walkover',
  );
  ...
  return {
    ...
    totalPoints: communityPoints.filter((point) => CREDITED_REASONS.includes(point.reason || ''))
      .length,
...
function filterRankingSessions(sessions: Session[], filter: CommunityRankingFilter) {
  ...
  if (filter === 'season') {
    ...
    return finished.filter(
      (session) => new Date(`${session.date}T12:00:00`).getFullYear() === year,
    );
  }
  return finished;
}
...
export function getCommunityRanking(params: {
  ...
  const sessionGames = params.games.filter(
    (game) =>
      sessionIds.has(game.sessionId) && (game.status === 'finished' || game.status === 'walkover'),
  );
  ...
  communitySessions.forEach((session) =>
    session.selectedPlayerIds.forEach((playerId) => communityPlayerIds.add(playerId)),
  );
  getCommunityPlayers(params.communityId, params.players).forEach((player) =>
    communityPlayerIds.add(player.id),
  );
  ...
  const rows = Array.from(communityPlayerIds).map((playerId) => {
    ...
    const playerGames = sessionGames.filter(
      (game) => playerTeamIds.includes(game.teamAId) || playerTeamIds.includes(game.teamBId),
    );
    ...
    const attendances = communitySessions.filter((session) =>
