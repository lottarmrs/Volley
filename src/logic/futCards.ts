...
export function toFut(v: number): number {
  return Math.max(1, Math.min(99, Math.round(VUT_CONSTANTS.curveOffset + v * VUT_CONSTANTS.curveScale)));
}
...
export function generateFutStats(player: Player): FutStats {
  ...
  const versatility = Math.max(1, Math.min(5,
...
export function buildVutCard(player: Player, ctx: BuildVutCardContext): VutCard {
  ...
  // Achievements
  const playerStats = calculatePlayerStats(player, ctx.games, ctx.pointEvents, ctx.teams, ctx.sessions);
...
function buildAchievementContext(
  ...
  for (const session of finishedSessions) {
    ...
    const sessionGames = ctx.games.filter(g => g.sessionId === session.id && g.status === 'finished');
    ...
    const ratings = participants
      .map(p => ({
        id: p.id,
        rating: calculateSessionRating({ player: p, sessionGames, sessionPoints, teams: sessionTeams }),
      }))
      .filter(r => r.rating != null)
      .sort((a, b) => b.rating! - a.rating!);
...
function countGamesWithMinRating(player: Player, ctx: BuildVutCardContext, minRating: number): number {
  let count = 0;
  const finishedGames = ctx.games.filter(g => g.status === 'finished');
  for (const game of finishedGames) {
    const team = ctx.teams.find(t =>
...
export const ACHIEVEMENT_CATALOG: AchievementDef[] = [
  ...
  {
    id: 'shared_cria', name: 'Cria da Quadra', emoji: '🏠', rarity: 'common', position: 'all',
  ...
  {
    id: 'shared_rodou', name: 'Rodou a Bola', emoji: '🔄', rarity: 'uncommon', position: 'all',
  ...
  {
    id: 'shared_presenca', name: 'Presença Garantida', emoji: '✅', rarity: 'common', position: 'all',
  ...
  {
    id: 'shared_ponto', name: 'Ponto Feito', emoji: '⚡', rarity: 'uncommon', position: 'all',
  ...
  {
    id: 'shared_cestinha', name: 'Cestinha', emoji: '🏅', rarity: 'rare', position: 'all',
  ...
  {
    id: 'shared_placar', name: 'Bom de Placar', emoji: '📊', rarity: 'rare', position: 'all',
  ...
  {
    id: 'shared_quente', name: 'Sequência Quente', emoji: '🔥', rarity: 'epic', position: 'all',
  ...
  {
    id: 'shared_craque', name: 'Craque da Noite', emoji: '👑', rarity: 'legendary', position: 'all',
  ...
  {
    id: 'shared_qualquer', name: 'Joga em Qualquer Lugar', emoji: '🔀', rarity: 'rare', position: 'all',
    ...
    condition: (_s, c) => {
      ...
      return rec.allPositions.filter(p => p.rating >= best - VUT_CONSTANTS.versatilityGap).length >= 4;
    },
    progress: (_s, c) => {
      const rec = getPlayerRecommendation(c.player);
      const best = rec.allPositions[0].rating;
      return {
        current: rec.allPositions.filter(p => p.rating >= best - VUT_CONSTANTS.versatilityGap).length,
        target: 4,
      };
    },
  },
  {
    id: 'shared_saldo', name: 'Mais Acerta que Erra', emoji: '➕', rarity: 'uncommon', position: 'all',
    frame: frame('f_saldo', 'Verde Limpo', 'uncommon', 'clean-green'),
    condition: (s) => s.balance >= 50,
    progress: (s) => ({ current: s.balance, target: 50 }),
  },
  {
    id: 'shared_selecao', name: 'Nível Seleção', emoji: '💎', rarity: 'legendary', position: 'all',
  ...
  {
    id: 'shared_sempre', name: 'Sempre no Jogo', emoji: '🫡', rarity: 'common', position: 'all',
  ...
  // ── Levantador ──────────────────────────────────────────────────────────
  {
    id: 'lev_bom', name: 'Bom de Bola', emoji: '🤲', rarity: 'common', position: 'levantador',
    frame: frame('f_lev_bom', 'Traço Fino', 'common', 'thin-line'),
    condition: (s) => s.assists >= 10,
    progress: (s) => ({ current: s.assists, target: 10 }),
  },
  {
    id: 'lev_dono', name: 'Dono do Jogo', emoji: '🎯', rarity: 'rare', position: 'levantador',
  ...
  {
    id: 'lev_cerebro', name: 'Cérebro da Quadra', emoji: '🧠', rarity: 'epic', position: 'levantador',
  ...
  {
    id: 'lev_cara', name: 'O Cara do Passe', emoji: '🎭', rarity: 'rare', position: 'levantador',
  ...
  {
    id: 'lev_classe', name: 'Toque de Classe', emoji: '✨', rarity: 'uncommon', position: 'levantador',
  ...
  {
    id: 'lev_pontua', name: 'Levantador que Pontua', emoji: '🎲', rarity: 'uncommon', position: 'levantador',
  ...
  {
    id: 'lev_firme', name: 'Mão Firme', emoji: '🛡️', rarity: 'rare', position: 'levantador',
  ...
  {
    id: 'lev_engrenagem', name: 'Engrenagem do Time', emoji: '⚙️', rarity: 'common', position: 'levantador',
    frame: frame('f_lev_engrenagem', 'Mecânica', 'common', 'mechanic'),
    condition: (s) => s.assists >= 10 && s.gamesPlayed >= 20,
    progress: (s) => ({ current: s.assists, target: 10 }),
  },
  {
    id: 'lev_frio', name: 'Maestro Frio', emoji: '🧊', rarity: 'epic', position: 'levantador',
  ...
  {
    id: 'lev_batuta', name: 'Batuta', emoji: '🎼', rarity: 'legendary', position: 'levantador',
  ...
  // ── Oposto ──────────────────────────────────────────────────────────────
  {
    id: 'opo_braco', name: 'Braço Pesado', emoji: '💢', rarity: 'common', position: 'oposto',
  ...
  {
    id: 'opo_canhao', name: 'Bola de Canhão', emoji: '🔥', rarity: 'rare', position: 'oposto',
  ...
  {
    id: 'opo_sem', name: 'Sem Defesa', emoji: '💣', rarity: 'epic', position: 'oposto',
  ...
  {
    id: 'opo_decisivo', name: 'Decisivo', emoji: '🎯', rarity: 'rare', position: 'oposto',
  ...
  {
    id: 'opo_impacto', name: 'Oposto de Impacto', emoji: '🦾', rarity: 'uncommon', position: 'oposto',
  ...
  {
    id: 'opo_completo', name: 'Ataque Completo', emoji: '🌊', rarity: 'epic', position: 'oposto',
  ...
  {
    id: 'opo_dono', name: 'Dono da Rede', emoji: '🐆', rarity: 'legendary', position: 'oposto',
  ...
  // ── Ponteiro ────────────────────────────────────────────────────────────
  {
    id: 'pon_passador', name: 'Passador Seguro', emoji: '🤝', rarity: 'common', position: 'ponteiro',
  ...
  {
    id: 'pon_recebe_mata', name: 'Recebe e Mata', emoji: '⚡', rarity: 'rare', position: 'ponteiro',
  ...
  {
    id: 'pon_ponta', name: 'Ponta de Lança', emoji: '🌟', rarity: 'epic', position: 'ponteiro',
  ...
  {
    id: 'pon_viagem', name: 'Viagem de Saque', emoji: '🎯', rarity: 'uncommon', position: 'ponteiro',
  ...
  {
    id: 'pon_faz_tudo', name: 'Faz Tudo na Ponta', emoji: '🔄', rarity: 'epic', position: 'ponteiro',
  ...
  {
    id: 'pon_ferro', name: 'Recepção de Ferro', emoji: '🛡️', rarity: 'rare', position: 'ponteiro',
  ...
  {
    id: 'pon_nota', name: 'Nota Alta', emoji: '📈', rarity: 'uncommon', position: 'ponteiro',
  ...
  {
    id: 'pon_ganhou', name: 'Ganhou mais que Perdeu', emoji: '📊', rarity: 'common', position: 'ponteiro',
  ...
  {
    id: 'pon_coringa', name: 'Coringa de Ponta', emoji: '🃏', rarity: 'rare', position: 'ponteiro',
    frame: frame('f_pon_coringa', 'Carta Estilizada', 'rare', 'stylized-card'),
    condition: (s, c) => {
      const rec = getPlayerRecommendation(c.player);
      const best = rec.allPositions[0].rating;
      const vers = rec.allPositions.filter(p => p.rating >= best - VUT_CONSTANTS.versatilityGap).length;
      return vers >= 3 && s.totalPoints >= 40;
    },
    progress: (s) => ({ current: s.totalPoints, target: 40 }),
  },
  {
    id: 'pon_pipoqueiro', name: 'Pipoqueiro', emoji: '🍿', rarity: 'legendary', position: 'ponteiro',
  ...
  {
    id: 'cen_saida', name: 'Saída de Rede', emoji: '⚡', rarity: 'uncommon', position: 'central',
    frame: frame('f_cen_saida', 'Relâmpago Rápido', 'uncommon', 'quick-lightning'),
    condition: (s) => s.cortadas >= 30,
    progress: (s) => ({ current: s.cortadas, target: 30 }),
  },
  {
    id: 'cen_duplo', name: 'Bloqueio e Ataque', emoji: '💀', rarity: 'epic', position: 'central',
  ...
  {
    id: 'cen_xerife', name: 'Xerife da Rede', emoji: '🔒', rarity: 'rare', position: 'central',
  ...
  {
    id: 'cen_saque', name: 'Central com Saque', emoji: '💥', rarity: 'uncommon', position: 'central',
  ...
  {
    id: 'cen_torre', name: 'Torre', emoji: '🗼', rarity: 'rare', position: 'central',
  ...
  {
    id: 'cen_dominou', name: 'Dominou a Rede', emoji: '👁️', rarity: 'epic', position: 'central',
  ...
  {
    id: 'cen_selecao', name: 'Central de Seleção', emoji: '🦁', rarity: 'legendary', position: 'central',
  ...
  // ── Líbero ──────────────────────────────────────────────────────────────
  {
    id: 'lib_chao', name: 'Bom de Chão', emoji: '🤲', rarity: 'common', position: 'libero',
  ...
  {
    id: 'lib_nao_cai', name: 'Não Cai Aqui', emoji: '🧱', rarity: 'rare', position: 'libero',
  ...
  {
    id: 'lib_gato', name: 'Gato', emoji: '🐱', rarity: 'epic', position: 'libero',
  ...
  {
    id: 'lib_barreira', name: 'Última Barreira', emoji: '🦸', rarity: 'rare', position: 'libero',
    frame: frame('f_lib_barreira', 'Capa de Herói', 'rare', 'hero-cape'),
    condition: (_s, c) => c.muralhaCount >= 3,
    progress: (_s, c) => ({ current: c.muralhaCount, target: 3 }),
  },
  {
    id: 'lib_seguranca', name: 'Segurança do Time', emoji: '🤫', rarity: 'uncommon', position: 'libero',
    frame: frame('f_lib_seguranca', 'Sombra Elegante', 'uncommon', 'elegant-shadow'),
    condition: (s, c) => s.highlights >= 20 && (c.formAvg ?? 0) >= 7.0,
    progress: (s) => ({ current: s.highlights, target: 20 }),
  },
  {
    id: 'lib_paredao', name: 'Paredão de Fundo', emoji: '🏗️', rarity: 'rare', position: 'libero',
  ...
  {
    id: 'lib_nivel', name: 'Líbero de Nível', emoji: '⭐', rarity: 'epic', position: 'libero',
  ...
  {
    id: 'lib_passe_a', name: 'Passe A', emoji: '💎', rarity: 'epic', position: 'libero',
  ...
  {
    id: 'lib_sempre', name: 'Sempre na Quadra', emoji: '📅', rarity: 'rare', position: 'libero',
    frame: frame('f_lib_sempre', 'Calendário Brilho', 'rare', 'shining-calendar'),
    condition: (s, c) => c.sessionsAttended >= 30 && s.highlights >= 30,
    progress: (_s, c) => ({ current: c.sessionsAttended, target: 30 }),
  },
  {
    id: 'lib_lendario', name: 'Lendário de Fundo', emoji: '🛡️', rarity: 'legendary', position: 'libero',
  ...
  // ── All-Rounder ─────────────────────────────────────────────────────────
  {
    id: 'all_quebra', name: 'Quebra-Galho', emoji: '🔧', rarity: 'common', position: 'all-rounder',
    frame: frame('f_all_quebra', 'Ferramenta Limpa', 'common', 'clean-tool'),
    condition: (s) => s.totalPoints >= 10 && s.assists >= 5 && s.highlights >= 5,
    progress: (s) => ({ current: Math.min(s.totalPoints, s.assists * 2, s.highlights * 2), target: 10 }),
  },
  {
    id: 'all_resolve', name: 'Resolve', emoji: '🔪', rarity: 'rare', position: 'all-rounder',
    frame: frame('f_all_resolve', 'Lâmina Afiada', 'rare', 'sharp-blade'),
    condition: (s) => s.totalPoints >= 30 && s.assists >= 15 && s.highlights >= 15,
    progress: (s) => ({ current: Math.min(s.totalPoints / 2, s.assists, s.highlights), target: 15 }),
  },
  {
    id: 'all_tapa', name: 'Tapa Buraco de Luxo', emoji: '🃏', rarity: 'rare', position: 'all-rounder',
  ...
  {
    id: 'all_pouco', name: 'Um Pouco de Cada', emoji: '⚖️', rarity: 'epic', position: 'all-rounder',
    ...
    progress: (s) => ({ current: Math.min(s.cortadas, s.blocks, s.aces, s.highlights), target: 10 }),
  },
  {
    id: 'all_camaleao', name: 'Camaleão', emoji: '🦎', rarity: 'uncommon', position: 'all-rounder',
  ...
  {
    id: 'all_cola', name: 'Cola do Time', emoji: '🧲', rarity: 'rare', position: 'all-rounder',
    frame: frame('f_all_cola', 'Magneto Coesivo', 'rare', 'cohesive-magnet'),
    condition: (s) => s.winRate >= 60 && s.gamesPlayed >= 30,
    progress: (s) => ({ current: Math.round(s.winRate), target: 60 }),
  },
  {
    id: 'all_panelinha', name: 'Jogador de Panelinha', emoji: '🏐', rarity: 'common', position: 'all-rounder',
  ...
  {
    id: 'all_polivalente', name: 'Polivalente', emoji: '🌈', rarity: 'epic', position: 'all-rounder',
    frame: frame('f_all_polivalente', 'Espectro Arco-Íris', 'epic', 'rainbow-spectrum'),
    condition: (s) => s.aces >= 5 && s.blocks >= 5 && s.cortadas >= 10 && s.assists >= 5 && s.highlights >= 10,
    progress: (s) => ({
  ...
  {
    id: 'all_hora', name: 'Na Hora Certa', emoji: '🎰', rarity: 'uncommon', position: 'all-rounder',
  ...
  {
    id: 'all_faz_jogar', name: 'Faz o Time Jogar', emoji: '👻', rarity: 'legendary', position: 'all-rounder',
...
export const ACHIEVEMENT_DESCRIPTIONS: Record<string, string> = {
  ...
  opo_pontas: 'Marcar pelo menos 50 pontos de ataque (cortadas) e no mínimo 20 pontos de saque (aces)',
  ...
  opo_dono: 'Manter contribuição média de pontos por jogo igual ou superior a 25% (mínimo de 30 jogos)',

  // Ponteiro
  pon_passador: 'Conquistar pelo menos 20 lances de destaque (highlights) de recepção',
  pon_recebe_mata: 'Marcar pelo menos 30 pontos de ataque (cortadas) e obter no mínimo 15 highlights de recepção',
  pon_ponta: 'Marcar pelo menos 60 pontos de ataque (cortadas) e obter no mínimo 30 highlights gerais',
  ...
  lib_seguranca: 'Obter no mínimo 20 highlights gerais e manter nota média de forma igual ou maior que 7.0',
  ...
  all_polivalente: 'Marcar no mínimo 5 aces, 5 bloqueios, 10 cortadas, dar 5 assistências e obter 10 highlights',
  all_hora: 'Ter pelo menos 3 jogos disputados com nota individual igual ou superior a 8.0',
  all_faz_jogar: 'Dar no mínimo 15 assistências, obter 20 highlights e manter saldo positivo mínimo de 20',
};
...
export function resolveAchievements(
  ...
  const applicable = ACHIEVEMENT_CATALOG.filter(
    a => a.position === 'all' || a.position === pos,
  );
