...
export function calculatePlayerStats(
  ...
  const registeredGames = allGames.filter(game =>
    game.status === 'finished' &&
    registeredSessionIds.has(game.sessionId)
  );
  ...
  const playerGames = registeredGames.filter(g => 
  ...
  const playerPoints = allPoints.filter(p => p.playerId === player.id && registeredGameIds.has(p.gameId));
  
  const CREDITED_REASONS = ['attack', 'block', 'serve_ace', 'defense_counterattack', 'tip'] as const;
  ...
  const errorsPoints = allPoints.filter(p => 
    registeredGameIds.has(p.gameId) &&
    p.concedingTeamId && teamIdentity.includes(p.concedingTeamId) && 
