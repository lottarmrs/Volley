...
begin
  ...
  -- A ativacao das comunidades de ORIGEM e verificada dentro da instrucao unica adiante.
  -- Esta, a da propria Session, fica fora sem risco: cutovers e insert-only (nao existe
  -- caminho de desativacao) e a Session esta travada, entao a divisao so pode ampliar a
  -- aceitacao entre as duas leituras, nunca produzir vetor de instantes misturados.
  if not exists (
    select 1
  ...
  with entry as (
    ...
    select e.entry_order
     ...
     where e.identity_kind = 'PLAYER'
       and (
         e.player_id is null
         -- Vinculo vivo vem do helper canonico, nao de um predicado proprio: ele ja
         -- existe justamente para que a checagem de W4-06 e a de W4-04 nao divirjam, e
         -- e `stable`, entao continua dentro do snapshot desta instrucao. Um participante
         -- admitido por predicado errado fica congelado para sempre neste artefato.
         or not app_private.registration_player_standing_alive(
              v_session.community_id, e.player_id
            )
       )
    ...
    -- Mesmo filtro da validacao de deriva acima. A coluna `kind` existe para que o
    -- `check (kind = 'SOURCE')` possa ser afrouxado; sem este filtro, a primeira dimensao
    -- nao-SOURCE de v0-legacy-11 passaria pela guarda de deriva e o vetor ganharia uma
    -- chave que o solver nunca viu -- exatamente o que a guarda existe para impedir.
    ...
    select d.dimension_key,
           d.display_order,
           case
             when pg_catalog.count(o.value) = 0 then 5.0::numeric
    ...
    select e.entry_order,
           pg_catalog.jsonb_build_object(
             ...
             -- numeric aceita NaN e +/-Infinity, e to_jsonb emite os tres como STRING
             -- JSON -- quebrando `height_cm: number | null` do cliente. `<= 0` nao pega
             -- NaN nem +Infinity (NaN ordena acima de tudo no Postgres). Este predicado
             -- unico cobre os quatro casos sem inventar altura maxima.
             'height_cm', case
               when e.identity_kind <> 'PLAYER' then null
               when p.height is null then null
               when not (p.height > 0::numeric and p.height < 'Infinity'::numeric) then null
